...
function EditLetter(ctr_This, LetterID)
{
    LetterEdit = LetterID;

    var url = Domain + "/MyVlive.EditLetter.ajax";
...
function UpdateLetter(ctr_This, LetterID)
{
    LetterID_Current = LetterID;
    
    var url = Domain + "/MyVlive.SaveLetter.ajax";

    var PushTime = $("#tbx_Day_U").val() + "T" + $("#sel_Hour_U").val() + ":" + $("#sel_Minute_U").val() + ":" + $("#sel_Second_U").val();
    ...
    try
    {
        ...
        $.ajax({
            type: "POST", //Phương thức gửi request là POST hoặc GET
            data: "LetterID=" + LetterID_Current + "&NewsID=" + NewsID + "&LetterName=" + $("#tbx_LetterName_U").attr("value") + "&Priority=" + $("#tbx_Priority_U").attr("value") + "&PushTime=" + PushTime, //tham số gửi kèm
            url: url, //Đường dẫn tới nơi xử lý request ajax
            success: function(string)
            {
                debugger;
