...
{
    public partial class Ad_GenerationXML : System.Web.UI.Page
    {
        ...
        int Admin_Paging1_GetTotalPage_Callback_Change()
        {
            try
            {
                ...
                return mGen.TotalRow(SearchType, str_SearchContent, CateID, IsActive);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        DataTable Admin_Paging1_GetData_Callback_Change()
        {
            try
            {
                ...
                return mGen.Search(SearchType, Admin_Paging1.mPaging.BeginRow, Admin_Paging1.mPaging.EndRow, str_SearchContent, CateID, IsActive, SortBy);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        protected void lbtn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mGen.Delete(0, dds_Parent.GetXml()))
                {
                    #region Log member
                    MemberLog mLog = new MemberLog();
                    MemberLog.ActionType Action = MemberLog.ActionType.Delete;
                    mLog.Insert("Generation", string.Empty, dds_Parent.GetXml(), Action, true, string.Empty);
                    #endregion
        ...
        private void Publish(bool IsActive)
        {
            try
            {
                char[] key_1 = { '|' };
                ...
                if (mGen.Active(0, IsActive, dds_Parent.GetXml()))
                {
                    #region Log member
                    MemberLog mLog = new MemberLog();
                    MemberLog.ActionType Action = IsActive ? MemberLog.ActionType.Active : MemberLog.ActionType.InActive;
                    mLog.Insert("GenerationXML", string.Empty, dds_Parent.GetXml(), Action, true, string.Empty);
                    #endregion
